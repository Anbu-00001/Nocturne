...
- Check a schema change against the phone's real database before installing:
  ...
  then `NOCTURNE_LIVE_DB=nocturne.db ./gradlew :data:testDebugUnitTest --tests '*LiveDatabaseTest*'` and read `data/build/live-database.txt`, which also lists the regularity windows for the latest night.
...
- **Every raw event stays.** The analytics spec's storage arithmetic matches this phone: about 1,830 events a day, and the whole database with derived tables was 2.9 MB after 11 days.
