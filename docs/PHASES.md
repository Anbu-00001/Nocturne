...
| 4 | ActivityWatch on Ubuntu: laptop use into sleep inference and §6.1, phone sessions and nights into ActivityWatch | laptop + 📱 over adb | installed on the A18 2026-09-17 00:22 (schema 7, sleep model 4); first sync 00:55 ok; a night with laptop use still to see |
...
- Enter sleep times from the laptop without touching the phone's screen (debug builds; only adb's shell can send it):
...
- Export everything from the phone without adb: Settings, "Export all data", writes one zip with every table as a CSV and `about.csv` (phone model, Android version, permissions, standby bucket, the packages the classifier treats specially, row counts). Gap note text is left out. On a debug build `adb shell am broadcast -n io.github.anbu00001.nocturne/.debug.DebugReportReceiver --es export all` writes the same zip to the app's files. On the A18's copy of 17 Sept: 27,350 rows in 15 tables, a 276 KB zip, in half a second.
...
## Nights entered from the laptop (2026-09-16)

- Opening the app on the phone to enter times would itself put screen events into the night being measured, so debug builds carry a receiver that only adb's shell can reach (it requires DUMP, which the shell holds and apps cannot get).
- Wed 9 (01:59 to 10:48) and Fri 11 (03:58 to 13:00), the times you had confirmed, went in at 01:27 on 16 Sept. Both nights now read as entered, and with 7 usable nights the evening window is personal: 00:10 to 12:23, starting 3 h before your usual onset.

## Phase 3b: the circadian model (2026-09-16)

- **The port.** `core-model`'s `circadian` package holds Hannay19 (single population) and Forger99, written in the reference package's own operation order, with its fixed-step RK4 (holding the light at the end of each step) and its core-temperature-minimum rule (SciPy peaks at least 13 h apart, DLMO 7 h earlier). Against Arcascope `circadian` 1.0.3 on an office week and a late sleeper's dim week at 6-minute steps, and two days at 1-minute steps, every state matches within 1e-9 (the spec asks 1e-4) and every DLMO time is identical. SciPy's peak picking is ported with its plateau and distance rules; when two peaks are exactly equal, SciPy's pick follows NumPy's vectorised sort and cannot be reproduced, which does not arise on a continuous signal. The real week of phone light the spec also asks for is added once light has run a full week (22 Sept).
- **Light input.** One photopic lux value a minute: 0 while asleep, a light sample's reading times the sensor-to-eye factor, the last reading for 30 min after the screen goes off, and otherwise an assumed level for daylight hours (06:00 to 18:00) or after dark. Four scenarios: dim day with bright evening (50 and 150 lux), middle (200 and 30, from free-living wearable studies and domestic evening light), bright day with dim evening (1000 and 1), and the medians of your own readings.
- **Starting state.** The model starts entrained to your habitual sleep in the first week: 30 days of dark while asleep and 800 lux otherwise, as Huang et al. (SLEEP 2021) started everyone from 16 h of 800 lux. The first attempt looped the first week from the package's default state instead; in dim light the model barely entrains, runs free near 24.2 h and drifted to a DLMO at 11:52.
- **Plausibility.** In free-living adults, model-predicted DLMO comes a mean of about 3 h before sleep onset, SD 1.4 to 2.2 h. A scenario whose DLMO is not between 1 h after and 7 h before the week's habitual onset is set aside, and a night is settled only when two or more of the rest agree within 2 h. This uses your sleep to judge the light assumptions, so a settled DLMO is not independent of your sleep.
...
Installed on the A18 at 16:12 on 16 Sept, after the pre-install check on a fresh copy of its database (schema 5 to 6, all 21,710 raw events kept; rollback APK and database backup saved outside the repository). On the phone the database reached schema 6 with the exported identity hash, raw_events_v4 was gone and nothing crashed.
...
- **The timer.** 15, 25, 45 or 60 min of focus and 5, 10 or 15 min of break, 25 and 5 by default. The end is an alarm, not a running service, so it survives the process being killed; a reboot clears alarms, so the app finishes or re-arms the timer when it next starts. A `shortService` foreground service would be stopped after 3 min. `USE_EXACT_ALARM` is granted at install from Android 13 (Google Play limits it to timer and alarm apps; Nocturne is not on Play), `SCHEDULE_EXACT_ALARM` covers Android 12 and earlier, and without either the alarm falls back to an inexact one and the tab says so.
- **Alarm clock, because ColorOS widens exact alarms.** The first build used `setExactAndAllowWhileIdle`. On the A18, `dumpsys alarm` showed Nocturne's alarm stored with a window of 75% of the time left (90 s for a 2 min block, 135 s for 3 min), Android's rule for inexact alarms, while system and alarm-app exact alarms kept a zero window; the 2 min block ended 90 s late, at the very end of that window. The app's log confirmed the exact call was made, so ColorOS's alarm service changed it. `setAlarmClock` kept a zero window: a 2 min block ended 2 ms late with the screen on, and 14 ms late in forced deep Doze (unplugged, screen off). Scaled up, the exact alarm could have ended a 25 min block about 19 min late. The cost is the alarm icon in the status bar while a timer runs, and the timer's end listed as the next alarm.
- **The signal.** A vibration marked as an alarm, since Android ignores other vibrations from a background app, plus the notification sound, which follows silent mode. On the A18 `dumpsys vibrator_manager` showed it played in full with usage ALARM, and the sound played on the notification stream. If notifications are allowed, a quiet notification then states the block's unlocks. A timer that ended while the phone was off is recorded without buzzing.
...
- **Standby buckets.** An app in Android's restricted bucket may set one alarm a day, exact or not; apps exempt from battery optimisation are outside the buckets. The Focus tab says so when the exemption is missing. The A18's allow-while-idle quota is 72 an hour (`dumpsys alarm`), not the 9 min spacing older guides give.
- **The gap length, checked.** On 12 days of the A18's history, gaps of at least 30, 45, 60, 90 and 120 min outside sleep and the evening window came to a median of 4, 3, 2, 0 and 0 a day (49, 34, 23, 8 and 3 in all). 60 min stays: about two candidates a day sits under the cap of four, and the weekly list holds about 13 rather than about 20.
- **Checked on the phone, 16 Sept.** Blocks of 1 to 3 min started and stopped from the debug receiver were recorded with the right lengths, "completed" false for the stopped one, and 0 unlocks, the phone not being used; they were then deleted from the history. `--es gaps preview` found no card (the latest gap had ended 16 h before) and 19 unlabelled gaps that week. Still to see: a real block with unlocks, and a card after a real gap.

Phone checks for 3a, after install (debug receiver commands in "Building"; none touches the screen):
...
- Permissions: `adb shell dumpsys package io.github.anbu00001.nocturne | grep -E "EXACT_ALARM|VIBRATE"` shows USE_EXACT_ALARM granted, and `--es focus state` reports exact alarms allowed.
- A 2 min block from the receiver: `adb shell dumpsys alarm | grep -A3 nocturne` lists one RTC_WAKEUP alarm with windowLength 0; at the end `adb shell dumpsys vibrator_manager` shows an alarm-usage vibration from Nocturne and `--es focus list` shows the block.
- The same in Doze: screen off, unplugged or `adb shell dumpsys battery unplug`, `adb shell dumpsys deviceidle force-idle`, start a block, check it ends on time, then `dumpsys deviceidle unforce` and `dumpsys battery reset`.
- A 15 min block with the screen off and the phone still, by hand: the vibration comes within a minute of the end, and the count matches the unlocks made during it.
...
- **Checked on the laptop, 16 and 17 Sept.** Model 4 with the A18's display profile reproduced all 13 stored nights exactly except 4 Sept's stale window, on a fresh copy of the phone's database (schema 6 to 7, all 22,268 raw events kept). Tonight's first real laptop use (00:06 and 00:16) imported into that copy and moved no night, being before the evening window.
- **Checked on the phone, 17 Sept.** Installed 00:22: schema 7, every raw event kept (22,268 then, 22,439 with the harvests since), all 13 nights re-derived under model run 3.4.2 with 4 Sept's window corrected, harvests OK at 00:31, 00:46 and 00:54, 56 light samples by 00:53 and the light service in the foreground, no crashes. The first `sync` at 00:55 sent 2 spans through `content write`, the phone reported the import ok with 2 nights re-derived, its database held both spans and the laptop's panel, and ActivityWatch received 1,124 phone sessions and 11 sleeps. Still to see: a night with laptop use, and the timeline and gap-card lines on screen (the build carrying them and the shift sentinel is not installed yet: Nocturne was open on the phone).

- **A moon in the top bar (17 Sept).** `tools/activitywatch/nocturne_indicator.py` puts Nocturne beside Ubuntu's system icons through AppIndicators (StatusNotifierItem), which Ubuntu 24.04 ships enabled with the Python bindings installed: no GNOME Shell extension, no log-out on Wayland (a new extension loads only after one), nothing to port when GNOME's extension API changes (all extensions had to move to ES modules in GNOME 45). The moon is dim with the evening window's start beside it, bright with a star inside the window, and faded with an amber dot when presence is not being recorded, which is the case it exists for: awatcher stopped silently on its first night here. Its menu lists the window, time at the laptop since noon and how much of it fell in tonight's window, the backlight and Night Light, the phone's latest night and when the phone last synced, with "Sync with the phone now". It never notifies. What it shows is `tonight.py`, pure and tested; `sync` leaves the phone's window and latest night in `~/.local/state/nocturne/phone.json` (the nights export now carries the window). On the laptop it registered with the shell, showed the right icon for 01:14 inside a 00:34 window, and used 53 MB and under 1% CPU. Ambient displays can help (UbiFit Garden's glanceable display, 12 people over 3 weeks) and fade once the novelty wears off; this one's lasting job is the not-recording warning, and the rest is there when clicked.
...
systemctl --user daemon-reload && systemctl --user enable --now aw-server.service nocturne-watch.service nocturne-sync.timer nocturne-indicator.service
```
(Already unpacked on this laptop on 17 Sept.) The web UI is at http://127.0.0.1:5600. To stop: `systemctl --user disable --now nocturne-indicator.service nocturne-sync.timer nocturne-watch.service aw-server.service`.
